...
enum L10n {
    static func text(_ key: L10nKey, language: AppLanguage) -> String {
        ...
        case (.printSettingsHint, .chinese): return "PDF 和图片会优先使用这些设置；Word 文档会优先通过 Microsoft Word 打印，找不到时再尝试 Pages/LibreOffice。"
        case (.printSettingsHint, .english): return "PDF and images use these settings first; Word documents print through Microsoft Word first, then Pages or LibreOffice."
        ...
        case (.officeCapabilityMessage, .chinese): return "Word 文档支持通过 Microsoft Word、Pages 或 LibreOffice 打印。"
        case (.officeCapabilityMessage, .english): return "Word documents can print through Microsoft Word, Pages, or LibreOffice."
